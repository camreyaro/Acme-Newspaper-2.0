...
public interface SuscriptionVolumenRepository extends JpaRepository<SuscriptionVolumen, Integer> {
	...
	@Query("select sv from SuscriptionVolumen sv join sv.volumen v join v.newspapers ns where sv.customer.id=?1 AND (select n from Newspaper n where n.id=?2) in ns")
